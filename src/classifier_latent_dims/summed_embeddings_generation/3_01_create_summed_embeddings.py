...
def write_h5_data(summed_embeddings_data: List[Tuple[np.ndarray, str, str]], output_file, walk_amount,
                  selected_cancers, latent_dim):
...
def sum_random_embeddings(patient_id: str, patient_data: Dict[str, np.ndarray], walk_distance: int,
                          walk_amount: int, modalities: List[str], latent_dim: int) -> np.ndarray:
    """
...
def main():
    ...
    parser.add_argument(
        "--load_path", "-l", type=str, default="results/embeddings/h5",
        help="Path to the embeddings folder."
    )
    parser.add_argument("--iteration", "-i", type=int, required=True)
    parser.add_argument("--latent_dim", "-ld", type=int, required=True,
    ...
    load_path = Path(load_path, str(latent_dim))
        ...
        for patient_id in tqdm(train_patient_ids, desc="Processing Patients"):
            ...
            try:
                summed_embedding = sum_random_embeddings(
                    ...
                    modalities=selected_modalities, latent_dim=latent_dim
                )
    ...
    write_h5_data(summed_train_embeddings_data, train_output_file, walk_amount, selected_cancers, latent_dim=latent_dim)
    write_h5_data(summed_test_embeddings_data, test_output_file, walk_amount, selected_cancers, latent_dim=latent_dim)
    ...
    write_h5_data(all_summed_embeddings_data, output_file, walk_amount, selected_cancers, latent_dim=latent_dim)
