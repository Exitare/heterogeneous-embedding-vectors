...
if __name__ == '__main__':
    ...
    for c1, df1 in cancer_dfs.items():
        for c2, df2 in cancer_dfs.items():
            if c1 == c2:
                continue
            inter = dot_product_distance(df1, df2).flatten()
            inter_distances[(c1, c2)] = inter

    y_lower = 0.0
    for cancer, distances in intra_distances.items():
        mean_val = float(np.mean(distances))
        y_lower = min(y_lower, mean_val)
        logging.info(f"Average intra-cancer distance for {cancer}: {mean_val:.4f}")

    for (c1, c2), distances in inter_distances.items():
        mean_val = float(np.mean(distances))
        y_lower = min(y_lower, mean_val)
        logging.info(f"Average inter-cancer distance for {c1} vs {c2}: {mean_val:.4f}")
    ...
    for (primary, df_list) in primary_cancers.items():
        ...
        for t in df["Type"].unique():
            y_lower = min(y_lower, float(df[df['Type'] == t]['Distance'].mean()))
    ...
    for i, (primary, df_list) in enumerate(primary_cancers.items()):
        ...
        axs[i].set_title(f'Dot-Product Distance for {primary}')
        axs[i].set_ylabel('Distance')
        axs[i].set_ylim(y_lower, 0)
