...
fig_save_folder = Path("results", "noise_recognizer", "distance_plots")
...
if __name__ == '__main__':
    ...
    summed_embeddings = pd.read_csv(
        Path("results", "noise_recognizer", "summed_embeddings", "multi", cancers, f"{walk_distance}_embeddings.csv"))

    fig_save_folder = Path(fig_save_folder, cancers, f"{walk_distance}_walk_distance_{len(summed_embeddings)}_walk_amount")
    ...
    plt.tight_layout()
    plt.savefig(Path(fig_save_folder, "all_combined_cosine_bar_plot.png"), dpi=150)
    plt.close('all')
