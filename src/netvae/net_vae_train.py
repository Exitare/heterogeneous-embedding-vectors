# !/usr/bin/env python

import os
import gzip
import json
import argparse
import pandas as pd
import networkx as nx
import numpy as np
from itertools import combinations

from scipy.special import softmax

import tensorflow as tf
from tensorflow import keras
from tensorflow.keras import layers


class Sampling(layers.Layer):
    """Uses (z_mean, z_log_var) to sample z"""

    def call(self, inputs):
        z_mean, z_log_var = inputs
        batch = tf.shape(z_mean)[0]
        dim = tf.shape(z_mean)[1]
        epsilon = tf.random.normal(shape=(batch, dim))
        return z_mean + tf.exp(0.5 * z_log_var) * epsilon


class VAE(keras.Model):
    def __init__(self, encoder, decoder, **kwargs):
        super().__init__(**kwargs)
        self.encoder = encoder
        self.decoder = decoder
        self.total_loss_tracker = keras.metrics.Mean(name="total_loss")
        self.reconstruction_loss_tracker = keras.metrics.Mean(
            name="reconstruction_loss"
        )
        self.kl_loss_tracker = keras.metrics.Mean(name="kl_loss")

    @property
    def metrics(self):
        return [
            self.total_loss_tracker,
            self.reconstruction_loss_tracker,
            self.kl_loss_tracker,
        ]

    def train_step(self, data):
        with tf.GradientTape() as tape:
            z_mean, z_log_var, z = self.encoder(data)
            reconstruction = self.decoder(z)

            """
            reconstruction_loss = tf.reduce_mean(
                tf.reduce_sum(
                    keras.losses.binary_crossentropy(data, reconstruction)
                )
            )

            kl_loss = -0.5 * (1 + z_log_var - tf.square(z_mean) - tf.exp(z_log_var))
            kl_loss = tf.reduce_mean(tf.reduce_sum(kl_loss, axis=1))
            total_loss = reconstruction_loss + kl_loss
            """
            reconstruction_loss = data.shape[1] * keras.metrics.binary_crossentropy(data, reconstruction)
            kl_loss = - 0.5 * tf.reduce_sum(1 + z_log_var - tf.square(z_mean) -
                                            tf.exp(z_log_var), axis=-1)
            # total_loss = tf.mean(reconstruction_loss + (tf.get_value(beta) * kl_loss))
            total_loss = reconstruction_loss + kl_loss

        grads = tape.gradient(total_loss, self.trainable_weights)
        self.optimizer.apply_gradients(zip(grads, self.trainable_weights))
        self.total_loss_tracker.update_state(total_loss)
        self.reconstruction_loss_tracker.update_state(reconstruction_loss)
        self.kl_loss_tracker.update_state(kl_loss)
        return {
            "loss": self.total_loss_tracker.result(),
            "reconstruction_loss": self.reconstruction_loss_tracker.result(),
            "kl_loss": self.kl_loss_tracker.result(),
        }


class NetTrainingCallback(tf.keras.callbacks.Callback):
    def __init__(self, netConstraint, phases):
        super().__init__()
        self.netConstraint = netConstraint
        self.phaseEnds = []
        self.nodeGroups = []
        self.nodeDFs = []
        s = 0
        for i in phases:
            s += i
            self.phaseEnds.append(s)

    def calc_netgroups(self):
        path_weights = pd.DataFrame(self.model.encoder.weights[0], index=subDF.columns)
        scoreDF = pd.DataFrame(softmax(path_weights.abs(), axis=1), index=path_weights.index,
                               columns=path_weights.columns)

        nodeDF = groupNodes(scoreDF, G)
        nodeGroup = {}
        for i in nodeDF:
            nodeGroup[i] = list(nodeDF.index[nodeDF[i] == 1.0])
        return nodeDF, nodeGroup

    def on_epoch_begin(self, epoch, logs):
        for i, j in enumerate(self.phaseEnds):
            if j == epoch:
                if (i % 2) == 0:
                    print("Start constraint phase")
                    nodeDF, nodeGroup = self.calc_netgroups()
                    self.nodeGroups.append(nodeGroup)
                    self.nodeDFs.append(nodeDF)
                    self.netConstraint.update_membership(nodeGroup)
                    self.netConstraint.set_active(True)
                else:
                    print("Start unconstrained phase")
                    nonZeroCount = len(np.nonzero(self.model.encoder.weights[0])[0])
                    print("non-zeros: %d" % (nonZeroCount))
                    self.netConstraint.set_active(False)


class NetworkConstraint(tf.keras.constraints.Constraint):
    def __init__(self, gene_index, pathway_index, pathway_membership):
        self.gene_index = gene_index
        self.pathway_index = pathway_index
        self.pathway_membership = pathway_membership
        self.active = True
        self.mask = tf.Variable(np.zeros([len(self.gene_index), len(self.pathway_index)]).astype("float32"))
        self.update()

    def update_membership(self, pathway_membership):
        self.pathway_membership = pathway_membership
        self.update()

    def set_active(self, a):
        self.active = a

    def update(self):
        if self.pathway_membership is not None:
            print("Updating mask")
            f = np.zeros([len(self.gene_index), len(self.pathway_index)]).astype("float32")
            gi = pd.Index(self.gene_index)
            for pi, p in enumerate(self.pathway_index):
                for g in self.pathway_membership[p]:
                    if g in gi:
                        f[gi.get_loc(g)][pi] = 1
            print("changing mask to custom")
            self.mask.assign(f)
        else:
            print("changing mask to ones")
            self.mask.assign(np.ones([len(self.gene_index), len(self.pathway_index)]).astype("float32"))

    def __call__(self, w):
        return w * self.mask

    def get_config(self):
        print("config", self.gene_index, self.pathway_index, self.pathway_membership)
        return {'gene_index': self.gene_index, "pathway_index": self.pathway_index,
                "pathway_membership": self.pathway_membership}


def build_encoder(feature_dim, latent_dim, constraint=None):
    # feature_dim = len(feature_index)
    # pathway_dim = len(pathway_index)
    encoder_inputs = keras.Input(shape=(feature_dim,), name="input_1")
    # constraint = None
    # if pathways is not None:
    #    constraint = NetworkConstraint(feature_index, pathway_index, pathways)
    pathway_layer = keras.layers.Dense(
        latent_dim,
        kernel_initializer='glorot_uniform',
        kernel_constraint=constraint,
        name="pathway_layer")(encoder_inputs)
    # x = keras.layers.Dense(
    #    latent_dim, kernel_initializer='glorot_uniform', name="encoder_dense_1")(pathway_layer)
    x = pathway_layer
    # latent_dim = pathway_dim
    x = keras.layers.BatchNormalization(name="batchnorm")(x)
    z_mean = layers.Dense(latent_dim, name="z_mean")(x)
    z_log_var = layers.Dense(latent_dim, name="z_log_var")(x)
    z = Sampling()([z_mean, z_log_var])
    encoder = keras.Model(encoder_inputs, [z_mean, z_log_var, z], name="encoder")

    print(encoder.summary())
    return encoder


def build_decoder(feature_dim, latent_dim):
    latent_inputs = keras.Input(shape=(latent_dim,))
    x = keras.layers.Dense(
        feature_dim, kernel_initializer='glorot_uniform',
        activation='sigmoid', name="decoder_input")(latent_inputs)
    decoder_outputs = x
    decoder = keras.Model(latent_inputs, decoder_outputs, name="decoder")
    print(decoder.summary())
    return decoder


def run_train(df, latent_dim,
              pathways=None,
              learning_rate=0.001, batch_size=128,
              phases=None
              ):
    feature_count = len(df.columns)

    pathway_index = list(range(latent_dim))

    constraint = NetworkConstraint(list(df.columns), pathway_index, pathways)
    constraint.set_active(False)

    encoder = build_encoder(len(df.columns), latent_dim, constraint)
    decoder = build_decoder(len(df.columns), latent_dim)

    vae = VAE(encoder, decoder)

    epochs = sum(phases)

    nc = NetTrainingCallback(constraint, phases)
    vae.compile(optimizer=keras.optimizers.Adam(learning_rate=learning_rate))
    history = vae.fit(df, epochs=epochs, batch_size=batch_size, shuffle=True,
                      callbacks=[nc]
                      )

    return encoder, decoder, history, {"groups": nc.nodeGroups, "dfs": nc.nodeDFs}


def groupNodes(valDF, G):
    subNets = {}
    for curGroup in valDF.columns:
        allV = {}
        groupStart = valDF.index[valDF.idxmax(axis=1) == curGroup]
        for i in groupStart:
            allV[i] = 1
        for i, j in combinations(groupStart, 2):
            # dist, path = nx.single_source_dijkstra(G, i)
            try:
                path = nx.shortest_path(G, i, j)
                for p in path:
                    allV[p] = 1
            except nx.NetworkXNoPath:
                pass
        subNets[curGroup] = allV
    subDF = pd.DataFrame(subNets, index=valDF.index).fillna(0.0)
    return subDF


if __name__ == "__main__":
    parser = argparse.ArgumentParser()

    parser.add_argument("matrix")
    parser.add_argument("pathway")
    parser.add_argument("--out", "-o", default="model")
    parser.add_argument("--gene-mask", default=None, type=str)
    parser.add_argument("--latent-dim", "-d", default=500, type=int)
    parser.add_argument("--learning-rate", default=1e-4, type=float)
    parser.add_argument("--start-epochs", default=120, type=int)
    parser.add_argument("--phase-count", default=20, type=int)
    parser.add_argument("--phase-epochs", default=20, type=int)
    parser.add_argument("--batch-size", default=256, type=int)

    args = parser.parse_args()

    print("Loading gene expression matrix")
    df = pd.read_csv(args.matrix, sep="\t", index_col=0)

    print("Loading pathway data")
    P = nx.MultiDiGraph()
    with gzip.open(args.pathway, "rt") as handle:
        for line in handle:
            row = line.rstrip().split("\t")
            if not (row[0].startswith("CHEBI:") or row[2].startswith("CHEBI:")):
                P.add_node(row[0])
                P.add_node(row[2])
                P.add_edge(row[0], row[2], label=row[1])
    G = nx.Graph(P)

    phases = [args.start_epochs, args.start_epochs] + ([args.phase_epochs] * (args.phase_count * 2))

    geneSet = df.columns.intersection(G.nodes)

    if args.gene_mask is not None:
        mask = pd.read_csv(args.gene_mask, header=None)[0]
        geneSet = geneSet.intersection(mask)

    subDF = df[geneSet]

    enc, dec, hist, nodeHist = run_train(subDF, latent_dim=args.latent_dim, learning_rate=args.learning_rate,
                                         phases=phases, batch_size=args.batch_size)

    if not os.path.exists(args.out):
        os.makedirs(args.out)
    enc.save(os.path.join(args.out, "model.enc"))
    dec.save(os.path.join(args.out, "model.dec"))

    with open(os.path.join(args.out, "net.groups.tsv"), "wt") as handle:
        for g, n in nodeHist["groups"][-1].items():
            handle.write("\t".join([str(g), *n]))
            handle.write("\n")

    with open(os.path.join(args.out, "stats.json"), "wt") as handle:
        handle.write(
            json.dumps({
                "loss_history": hist.history["loss"]
            }))

    with open(os.path.join(args.out, "index"), "wt") as handle:
        handle.write(json.dumps(list(geneSet)))