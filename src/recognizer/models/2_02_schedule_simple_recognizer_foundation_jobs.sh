...
do
  if [ -z "$local" ]
  then
    sbatch ./src/recognizer/models/2_01_run_simple_recognizer.sh "${cancer_types}" -1 $iteration $amount_of_summed_embeddings 0.0
  else
    ./src/recognizer/models/2_01_run_simple_recognizer.sh "${cancer_types}" -1 $iteration $amount_of_summed_embeddings 0.0
  fi
done