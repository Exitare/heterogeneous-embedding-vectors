...
if __name__ == '__main__':
    ...
    parser.add_argument("--amount_of_embeddings", "-a", type=int, required=True, help="The amount of embeddings to sum")
    args = parser.parse_args()

    selected_cancers = args.cancer
    walk_distance: int = args.walk_distance
    amount_of_embeddings: int = args.amount_of_embeddings
    ...
    summed_embeddings = pd.read_csv(
        Path("results", "recognizer", "summed_embeddings", "multi", cancers, str(amount_of_embeddings), "0.0",
             f"{walk_distance}_embeddings.csv"))
    ...
    for cancer in selected_cancers:
        ...
        assert "Mutation" not in cancer_rows.columns, "Mutation column is present"
