...
if __name__ == '__main__':
    ...
    for c1, df1 in cancer_dfs.items():
        for c2, df2 in cancer_dfs.items():
            if c1 == c2:
                continue
            inter = cosine_distances(df1, df2).flatten()
            inter_distances[(c1, c2)] = inter

    for cancer, distances in intra_distances.items():
        logging.info(f"Average intra-cancer cosine distance for {cancer}: {np.mean(distances):.4f}")

    for (c1, c2), distances in inter_distances.items():
        logging.info(f"Average inter-cancer cosine distance for {c1} vs {c2}: {np.mean(distances):.4f}")
    ...
    for (primary, df_list) in primary_cancers.items():
        ...
        for t in df["Type"].unique():
            y_upper = max(y_upper, float(df[df['Type'] == t]['Distance'].mean()))
    y_upper = y_upper + 0.5
    ...
    for i, (primary, df_list) in enumerate(primary_cancers.items()):
        ...
        axs[i].set_title(f'Cosine Distance for {primary}')
