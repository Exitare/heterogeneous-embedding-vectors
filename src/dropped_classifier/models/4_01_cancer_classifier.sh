...
if [ -z "$iteration" ]; then
  iteration=1
  echo "iteration is empty, setting to 1"
fi
...
# call script
python ./src/dropped_classifier/models/4_01_cancer_classifier.py -c ${cancers} -i ${iteration} -w ${walk_distance} -a ${walk_amount} --modalities ${modalities}
