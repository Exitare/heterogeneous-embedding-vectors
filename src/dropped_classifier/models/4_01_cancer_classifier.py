...
load_folder = Path("results", "dropped_classifier", "summed_embeddings")
save_folder = Path("results", "dropped_classifier", "classification")
...
if __name__ == "__main__":
    ...
    logging.info(f"Selected modalities: {modalities}")
    logging.info(f"Walk distance: {walk_distance}, Amount of walks: {walk_amount}")

    load_folder = Path(load_folder, cancers, f"{walk_distance}_{walk_amount}", modalities)
    h5_file_path = Path(load_folder, "summed_embeddings.h5")

    cancer_save_folder = Path(save_folder, cancers)
    cancer_save_folder = Path(cancer_save_folder, f"{walk_distance}_{walk_amount}", modalities)
