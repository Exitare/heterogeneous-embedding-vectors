...
def load_patient_data(h5_file: h5py.File, patient_id: str, modalities: [str]) -> Tuple[Dict[str, np.ndarray], str]:
    """
...
def collect_all_submitter_ids(h5_file: h5py.File, modalities: List[str]) -> List[str]:
    ...
    elif "annotations" in modalities:
        submitter_ids.update(h5_file["annotations"].keys())
        logging.info(f"Annotations modality: {len(submitter_ids)} submitter_ids collected.")
        return list(submitter_ids)
    else:
...
def main():
    ...
    parser.add_argument("--modalities", "-m", type=str, nargs="+",
                        default=["rna", "annotations", "mutations", "images"],
    ...
    modalities: str = "_".join(selected_modalities)
    load_path: Path = Path(args.load_path)

    save_folder = Path("results", "dropped_classifier", "summed_embeddings", cancers, f"{walk_distance}_{walk_amount}", modalities)
    ...
    with h5py.File(h5_load_path, "r") as h5_file:
        ...
        patient_ids = collect_all_submitter_ids(h5_file, selected_modalities)
        ...
        for patient_id in tqdm(patient_ids, desc="Processing Patients"):
            try:
                patient_data, patient_cancer = load_patient_data(h5_file, patient_id, modalities=selected_modalities)
